<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/productcategory/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/productcategory/create'"></button-create>
...
import { GET_STATE_PRODUCTCATEGORYS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.productcategory.productcategorys
    },
    pages () {
      return this.$store.state.productcategory.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.productcategory.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_PRODUCTCATEGORY', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_PRODUCTCATEGORYS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_PRODUCTCATEGORYS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_PRODUCTCATEGORYS)
      .then((result) => {
