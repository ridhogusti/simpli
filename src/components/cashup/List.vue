<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/cashup/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/cashup/create'"></button-create>
...
import { GET_STATE_CASHUPS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.cashup.cashups
    },
    pages () {
      return this.$store.state.cashup.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.cashup.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_CASHUP', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_CASHUPS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_CASHUPS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_CASHUPS)
      .then((result) => {
