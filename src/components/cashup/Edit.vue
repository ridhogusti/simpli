    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormCashUp :CashUp="cashup"></FormCashUp>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/cashup'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormCashUp from '@/components/cashup/Form'
import { GET_CASHUP, UPDATE_CASHUP, DELETE_CASHUP } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   cashup () {
      return this.$store.state.cashup.cashup
    }
  },
  mounted () {
    this.$store.dispatch(GET_CASHUP, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_CASHUP, this.id)
      .then((response) => {
        this.$router.push({ path: '/cashup' })
      }).catch((err) => {
        this.alert = {
          status: true,
          type: 'error',
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_CASHUP, this.cashup)
      .then((response) => {
