<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/productunit/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/productunit/create'"></button-create>
...
import { GET_STATE_PRODUCTUNITS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.productunit.productunits
    },
    pages () {
      return this.$store.state.productunit.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.productunit.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_PRODUCTUNIT', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_PRODUCTUNITS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_PRODUCTUNITS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_PRODUCTUNITS)
      .then((result) => {
