    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormMember :Member="member"></FormMember>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/member'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormMember from '@/components/member/Form'
import { GET_MEMBER, UPDATE_MEMBER, DELETE_MEMBER } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_MEMBER, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_MEMBER, this.id)
      .then((response) => {
