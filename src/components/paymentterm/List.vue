<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-else-if="index==1">
                       <router-link :to="'/paymentterm/edit/' + props.item.Id">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/paymentterm/create'"></button-create>
...
import { GET_STATE_PAYMENTTERMS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.paymentterm.paymentterms
    },
    pages () {
      return this.$store.state.paymentterm.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.paymentterm.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_PAYMENTTERM', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_PAYMENTTERMS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_PAYMENTTERMS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_PAYMENTTERMS)
      .then((result) => {
        console.log(result)
        this.headers.push({
          text: 'No',
