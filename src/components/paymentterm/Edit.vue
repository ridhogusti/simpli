    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormPaymentTerm :PaymentTerm="paymentterm"></FormPaymentTerm>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/paymentterm'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormPaymentTerm from '@/components/paymentterm/Form'
import { GET_PAYMENTTERM, UPDATE_PAYMENTTERM, DELETE_PAYMENTTERM } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   paymentterm () {
      return this.$store.state.paymentterm.paymentterm
    }
  },
  mounted () {
    this.$store.dispatch(GET_PAYMENTTERM, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_PAYMENTTERM, this.id)
      .then((response) => {
        this.$router.push({ path: '/paymentterm' })
      }).catch((err) => {
        this.alert = {
          status: true,
          type: 'error',
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_PAYMENTTERM, this.paymentterm)
      .then((response) => {
