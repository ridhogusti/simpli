    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormEmployeeItemShift :EmployeeItemShift="employeeitemshift"></FormEmployeeItemShift>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/employeeitemshift'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormEmployeeItemShift from '@/components/employeeitemshift/Form'
import { GET_EMPLOYEEITEMSHIFT, UPDATE_EMPLOYEEITEMSHIFT, DELETE_EMPLOYEEITEMSHIFT } from '@/store/actions.type'

export default {
  components: {
    FormEmployeeItemShift
  },
  props: ['id'],
  data () {
    return {
  ...
  computed: {
   employeeitemshift () {
      return this.$store.state.employeeitemshift.employeeitemshift
    }
  },
  mounted () {
    this.$store.dispatch(GET_EMPLOYEEITEMSHIFT, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_EMPLOYEEITEMSHIFT, this.id)
      .then((response) => {
        this.$router.push({ path: '/employeeitemshift' })
      }).catch((err) => {
        this.alert = {
          status: true,
          type: 'error',
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_EMPLOYEEITEMSHIFT, this.employeeitemshift)
      .then((response) => {
