<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/employeeitemshift/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/employeeitemshift/create'"></button-create>
...
import { GET_STATE_EMPLOYEEITEMSHIFTS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.employeeitemshift.employeeitemshifts
    },
    pages () {
      return this.$store.state.employeeitemshift.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.employeeitemshift.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_EMPLOYEEITEMSHIFT', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_EMPLOYEEITEMSHIFTS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_EMPLOYEEITEMSHIFTS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_EMPLOYEEITEMSHIFTS)
      .then((result) => {
