    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormDepartment :Department="department"></FormDepartment>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/department'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormDepartment from '@/components/department/Form'
import { GET_DEPARTMENT, UPDATE_DEPARTMENT, DELETE_DEPARTMENT } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_DEPARTMENT, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_DEPARTMENT, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_DEPARTMENT, this.department)
      .then((response) => {
