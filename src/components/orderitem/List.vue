<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/orderitem/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/orderitem/create'"></button-create>
...
import { GET_STATE_ORDERITEMS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.orderitem.orderitems
    },
    pages () {
      return this.$store.state.orderitem.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.orderitem.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_ORDERITEM', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_ORDERITEMS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_ORDERITEMS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_ORDERITEMS)
      .then((result) => {
