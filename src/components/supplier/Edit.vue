    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormSupplier :Supplier="supplier"></FormSupplier>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/supplier'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import { GET_SUPPLIER, UPDATE_SUPPLIER, DELETE_SUPPLIER } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_SUPPLIER, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_SUPPLIER, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_SUPPLIER, this.supplier)
      .then((response) => {
