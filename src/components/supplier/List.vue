<template>
  <div>
        ...
        <v-flex xs12 sm12 md12>
          <v-card>
            ...
            <v-flex>
              ...
              >
                <!-- header data table -->
                <template slot="headers" slot-scope="props" v-if="headers.length !== 0">
                  <tr class="">
                    <th
                      ...
                      class="text-md-left"
                    >
                      {{ header.text.match(/[A-Z][a-z]+|[0-9]+/g).join(" ") !== null?header.text.match(/[A-Z][a-z]+|[0-9]+/g).join(" "):header.text }}
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-else-if="index==1">
                       <router-link :to="'/supplier/edit/' + props.item.Id">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/supplier/create'"></button-create>
...
import { GET_STATE_SUPPLIERS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    ...
    pages () {
      return this.$store.state.supplier.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.supplier.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_SUPPLIER', page)
    },
    searchList (keyword) {
      this.$store.dispatch('GET_SEARCH_SUPPLIERS', keyword)
    },
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_SUPPLIERS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_SUPPLIERS)
      .then((result) => {
          this.headers.push({
            text: 'No',
            value: 'No'
