<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/purchaseitem/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/purchaseitem/create'"></button-create>
...
import { GET_STATE_PURCHASEITEMS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.purchaseitem.purchaseitems
    },
    pages () {
      return this.$store.state.purchaseitem.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.purchaseitem.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_PURCHASEITEM', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_PURCHASEITEMS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_PURCHASEITEMS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_PURCHASEITEMS)
      .then((result) => {
