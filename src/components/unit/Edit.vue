          ...
          <v-card-text>
            <v-form ref="form">
              ...
              <div class="botton-submit text-md-right">
                ...
                <v-btn round class="error" @click.native.stop="dialog = true"><v-icon left>delete</v-icon> Delete</v-btn>
              </div>
            </v-form>
          </v-card-text>
...
import { GET_UNIT, UPDATE_UNIT, DELETE_UNIT } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_UNIT, this.id)
  },
  methods: {
    deleted () {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_UNIT, this.unit)
      .then((response) => {
