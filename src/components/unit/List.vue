<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==1">
                       <router-link :to="'/unit/edit/' + props.item.UnitId">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/unit/create'"></button-create>
...
import { GET_STATE_UNITS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    ...
    pages () {
      return this.$store.state.unit.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.unit.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_UNIT', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_UNITS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_UNITS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_UNITS)
      .then((result) => {
        this.headers.push({
          text: 'No',
          value: 'No'
