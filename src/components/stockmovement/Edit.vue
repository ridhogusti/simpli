...
import { GET_STOCKMOVEMENT, UPDATE_STOCKMOVEMENT, DELETE_STOCKMOVEMENT } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   stockmovement () {
      return this.$store.state.stockmovement.stockmovement
    }
  },
  mounted () {
    this.$store.dispatch(GET_STOCKMOVEMENT, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_STOCKMOVEMENT, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_STOCKMOVEMENT, this.stockmovement)
      .then((response) => {
