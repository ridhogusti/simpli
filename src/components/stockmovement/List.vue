            ...
            <v-flex>
              ...
              >
                ...
                <template slot="items" slot-scope="props">
                  ...
                  <td v-if="props.item.MovementType == 0">In</td>
                  <td v-else-if="props.item.MovementType == 1">Out</td>
...
import { GET_STATE_STOCKMOVEMENTS } from '@/store/actions.type'
...
export default {
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'MovementType', value: 'MovementType'},
  ...
  computed: {
    items () {
      return this.$store.state.stockmovement.stockmovements
    },
    pages () {
      return this.$store.state.stockmovement.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.stockmovement.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_STOCKMOVEMENT', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_STOCKMOVEMENTS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_STOCKMOVEMENTS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_STOCKMOVEMENTS)
  }
}
</script>
