  ...
  <v-container fluid>
    <v-layout>
        ...
        <v-layout row wrap>
          <v-flex xs8>
            <v-select label="Supplier"
                      v-model="Purchase['SupplierId']" :items="$store.state.supplier.suppliers" item-text="Company"
                      v-on:change="() => {myFunctionSupplier(), setDueDate()}" item-value="Id"
                      :rules="true?[v => !!v || 'Supplier' + ' is required.']:[]" :required="true"
      ...
      <v-flex xs4>
        <v-layout row wrap>
          <v-flex xs8>
            <v-select label="Warehouse" v-model="Purchase['WarehouseId']" :items="$store.state.warehouse.warehouses"
                      item-text="Name" item-value="Id" :rules="true?[v => !!v || 'Warehouse' + ' is required.']:[]"
                      :required="true" autocomplete></v-select>
          </v-flex>
          <v-flex xs1>
        ...
        <v-layout row wrap>
          <v-flex xs8>
            <v-select label="Payment Term" v-model="Purchase['PaymentTermId']" v-on:change="() => {setDueDate()}"
                      :items="$store.state.paymentterm.paymentterms" item-text="Name" item-value="Id"
                      :rules="true?[v => !!v || 'Payment Term' + ' is required.']:[]" :required="true"
                      autocomplete></v-select>
          </v-flex>
          <v-flex xs1>
        ...
        <v-layout row wrap>
          <v-flex xs8>
            ...
            <v-menu ref="menu" :close-on-content-click="false" v-model="menu" :nudge-right="40" lazy
              ...
              <v-date-picker ref="picker" v-model="Purchase.DueDate" min="1950-01-01"
                             @change="saveDueDate"></v-date-picker>
            </v-menu>
          </v-flex>
        </v-layout>
      </v-flex>
        ...
        <v-layout row wrap>
          <v-flex xs8>
            <v-menu ref="menu1" :close-on-content-click="false" v-model="menu1" :nudge-right="40" lazy
              ...
              <v-date-picker ref="picker" v-model="Purchase.Date" min="1950-01-01"
                             @change="()=>{saveDate(); setDueDate() }"></v-date-picker>
            </v-menu>
          </v-flex>
        </v-layout>
        <v-layout row wrap>
          <v-flex xs8>
            <v-text-field label="Invoice No" value="-" v-model="Purchase['InvoiceNo']" type="text"
                          :rules="true?[v => !!v || 'Invoice No' + ' is required.']:[]" :required="true"></v-text-field>
          </v-flex>
        </v-layout>
      </v-flex>
    </v-layout>
    ...
    <v-data-table :headers="headers" :items="Purchase.ListPurchaseItem" hide-actions class="elevation-1">
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-right">
          <v-select :items="$store.state.product.products" v-model="Purchase['ListPurchaseItem'][props.index].ProductId"
                    item-text="Name" v-on:change="myFunction(props.index)" item-value="Id" autocomplete></v-select>
        </td>
        <td class="text-xs-right">
          <v-text-field :value="Purchase['ListPurchaseItem'][props.index].ProductDescription"
                        v-model="Purchase['ListPurchaseItem'][props.index].ProductDescription" name="description"
                        id="description"></v-text-field>
        </td>
        <td class="text-xs-right">
          <v-text-field :value="Purchase['ListPurchaseItem'][props.index].ProductPrice"
                        v-model="Purchase['ListPurchaseItem'][props.index].ProductPrice" name="price"
                        id="price"></v-text-field>
        </td>
        <td class="text-xs-right">
          <v-text-field :value="Purchase['ListPurchaseItem'][props.index].Quantity"
                        v-model="Purchase['ListPurchaseItem'][props.index].Quantity" name="quantity"
                        id="quantity"></v-text-field>
        </td>
        <td>
          <v-layout row wrap>
            <v-flex xs7>
              <v-text-field
                v-if="Purchase['ListPurchaseItem'][props.index].TipeDiscount == 'amount'"
                            :value="Purchase['ListPurchaseItem'][props.index].DiscountAmount"
                            v-model="Purchase['ListPurchaseItem'][props.index].DiscountAmount" name="discountAmount"
                            ></v-text-field>
              <v-text-field v-else-if="Purchase['ListPurchaseItem'][props.index].TipeDiscount == 'percentage'"
                            :value="Purchase['ListPurchaseItem'][props.index].DiscountPercentage"
                            v-model="Purchase['ListPurchaseItem'][props.index].DiscountPercentage"
                            name="discountPercentage"></v-text-field>
            </v-flex>
            <v-flex xs5 :style="{ marginTop: '10px'}">
              <v-btn-toggle v-model="Purchase['ListPurchaseItem'][props.index].ToggleDiscount" mandatory>
        ...
        <td>
          <v-layout row wrap>
            <v-flex xs7>
              <v-select v-model="Purchase['ListPurchaseItem'][props.index].Tax" :items="$store.state.tax.taxs"
                        item-text="Name" item-value="Name" :rules="true?[v => !!v || 'Tax' + ' is required.']:[]"
                        :required="true" autocomplete></v-select>
            </v-flex>
          </v-layout>
        </td>
          ...
          <v-flex xs6>
            <v-card>
              ...
              <v-card-text>
                <v-layout row wrap>
                  <v-flex xs5>
                    <v-text-field v-if="Purchase.TipeDiscount == 'amount'" :value="Purchase.DiscountAmount"
                                  v-model="Purchase.DiscountAmount" name="discountAmount" label="0"
                                  id="discountAmount"></v-text-field>
                    <v-text-field v-if="Purchase.TipeDiscount == 'percentage'" :value="Purchase.DiscountPercentage"
                                  v-model="Purchase.DiscountPercentage" name="discountPercentage" label="%"
                                  id="discountPercentage"></v-text-field>
                  </v-flex>
                  <v-flex xs5 :style="{ marginTop: '10px'}">
                    <v-btn-toggle v-model="Purchase.ToggleDiscount" mandatory>
        ...
        setTimeout(function () {
          self.$store
            ...
            .then(response => {
              ...
              self.Purchase.ListPurchaseItem[index].ProductPrice = response.data.PurchasePrice
