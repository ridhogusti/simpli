<template>
	<div>
		...
		<v-card class="retangle1">
			<v-card-text>
				<v-data-table :headers="headers" :items="GoodsReceipt.ListGoodsReceiptItem" hide-actions class="elevation-1 tablePurchase">
					<tr slot="items" slot-scope="props" style="background-color: !important">
						<td class="text-xs-right" style="width: 100px">
							{{ props.index + 1 }}
						</td>
						<td style="width: 300px !important; padding: 0px 10px !important;">
							<v-select :readonly="modereadonly"
								required 
								:rules="[() => !!GoodsReceipt['ListGoodsReceiptItem'][props.index].ProductUnitId || '']"
								:menu-props="{contentClass: 'listPurchase'}" class="select1" solo :items="productUnit" 
								v-model="GoodsReceipt['ListGoodsReceiptItem'][props.index].ProductUnitId" append-icon="expand_more" item-text="Name" v-on:change="myFunction(props.index)"
							...
							<!-- <v-autocomplete :readonly="modereadonly" :menu-props="{contentClass: 'listPurchaseOrder'}" class="select1" solo :items="$store.state.product.products" v-model="GoodsReceipt['ListPurchaseOrderitem'][props.index].ProductId"  item-value="Id" item-text="Name"  append-icon="expand_more" v-on:change="myFunction(props.index)"  persistent-hint>
									
								</v-autocomplete> -->
						</td>
	
						<td style="width: 170px !important; padding: 0px 10px !important">
							<v-text-field :readonly="modereadonly" 
								class="input2" solo :value="GoodsReceipt['ListGoodsReceiptItem'][props.index].ProductDescription"
								required  
								:rules="[() => !!GoodsReceipt['ListGoodsReceiptItem'][props.index].ProductDescription || '']"
								v-model="GoodsReceipt['ListGoodsReceiptItem'][props.index].ProductDescription" name="description" id="description"></v-text-field>
						</td>
		
						<td style="width: 146px !important; padding: 0px 10px !important" class="text-xs-right">
							<v-text-field :readonly="modereadonly" 
								required 
								class="input2 inputNumber" solo 
								:rules="[() => !!GoodsReceipt['ListGoodsReceiptItem'][props.index].Quantity || '']"
								:value="GoodsReceipt['ListGoodsReceiptItem'][props.index].Quantity" 
								v-model="GoodsReceipt['ListGoodsReceiptItem'][props.index].Quantity" 
								name="quantity" id="quantity"></v-text-field>
						</td>
				...
				<!-- <div v-else>
					<v-layout row wrap>
						<v-flex lg12>
							<v-btn block @click="addRow(GoodsReceipt.ListGoodsReceiptItem.length)" class="btnadd" color="primary" outline dark>
