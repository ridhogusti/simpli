<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/brand/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/brand/create'"></button-create>
...
import { GET_STATE_BRANDS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    ...
    pages () {
      return this.$store.state.brand.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.brand.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_BRAND', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_BRANDS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_BRANDS', sort)
    }
  },
  created () {
