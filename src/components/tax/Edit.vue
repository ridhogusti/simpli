...
import { GET_TAX, UPDATE_TAX, DELETE_TAX } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   tax () {
      return this.$store.state.tax.tax
    }
  },
  mounted () {
    this.$store.dispatch(GET_TAX, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_TAX, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_TAX, this.tax)
      .then((response) => {
