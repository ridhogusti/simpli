<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/tax/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/tax/create'"></button-create>
...
import { GET_STATE_TAXS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.tax.taxs
    },
    pages () {
      return this.$store.state.tax.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.tax.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_TAX', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_TAXS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_TAXS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_TAXS)
      .then((result) => {
