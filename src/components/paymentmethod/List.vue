<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/paymentmethod/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/paymentmethod/create'"></button-create>
...
import { GET_STATE_PAYMENTMETHODS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.paymentmethod.paymentmethods
    },
    pages () {
      return this.$store.state.paymentmethod.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.paymentmethod.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_PAYMENTMETHOD', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_PAYMENTMETHODS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_PAYMENTMETHODS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_PAYMENTMETHODS)
      .then((result) => {
