    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormPaymentMethod :PaymentMethod="paymentmethod"></FormPaymentMethod>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/paymentmethod'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormPaymentMethod from '@/components/paymentmethod/Form'
import { GET_PAYMENTMETHOD, UPDATE_PAYMENTMETHOD, DELETE_PAYMENTMETHOD } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   paymentmethod () {
      return this.$store.state.paymentmethod.paymentmethod
    }
  },
  mounted () {
    this.$store.dispatch(GET_PAYMENTMETHOD, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_PAYMENTMETHOD, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_PAYMENTMETHOD, this.paymentmethod)
      .then((response) => {
