...
import { GET_STOCK, UPDATE_STOCK, DELETE_STOCK } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_STOCK, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_STOCK, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_STOCK, this.stock)
      .then((response) => {
