<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/cashupitem/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/cashupitem/create'"></button-create>
...
import { GET_STATE_CASHUPITEMS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.cashupitem.cashupitems
    },
    pages () {
      return this.$store.state.cashupitem.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.cashupitem.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_CASHUPITEM', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_CASHUPITEMS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_CASHUPITEMS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_CASHUPITEMS)
      .then((result) => {
