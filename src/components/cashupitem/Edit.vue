...
import { GET_CASHUPITEM, UPDATE_CASHUPITEM, DELETE_CASHUPITEM } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   cashupitem () {
      return this.$store.state.cashupitem.cashupitem
    }
  },
  mounted () {
    this.$store.dispatch(GET_CASHUPITEM, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_CASHUPITEM, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_CASHUPITEM, this.cashupitem)
      .then((response) => {
