    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormCashUpItem :CashUpItem="cashupitem"></FormCashUpItem>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/cashupitem'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
<script>
import FormCashUpItem from '@/components/cashupitem/Form'
import { CREATE_CASHUPITEM } from '@/store/actions.type'
  ...
  methods: {
    ...
    save () {
      if (this.$refs.form.validate()) {
        ...
        this.$store.dispatch(CREATE_CASHUPITEM, this.cashupitem)
        .then((response) => {
          this.alert = {
            status: true,
            type: 'success',
          ...
          this.$router.push({ path: '/cashupitem' })
        }).catch((err) => {
          console.log('gagal')
          this.alert = {
            status: true,
