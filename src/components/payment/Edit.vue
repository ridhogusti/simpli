...
import { GET_PAYMENT, UPDATE_PAYMENT, DELETE_PAYMENT } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_PAYMENT, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_PAYMENT, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_PAYMENT, this.payment)
      .then((response) => {
