<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/productvariant/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/productvariant/create'"></button-create>
...
import { GET_STATE_PRODUCTVARIANTS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.productvariant.productvariants
    },
    pages () {
      return this.$store.state.productvariant.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.productvariant.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_PRODUCTVARIANT', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_PRODUCTVARIANTS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_PRODUCTVARIANTS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_PRODUCTVARIANTS)
      .then((result) => {
