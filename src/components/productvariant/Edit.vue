    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormProductVariant :ProductVariant="productvariant"></FormProductVariant>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/productvariant'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormProductVariant from '@/components/productvariant/Form'
import { GET_PRODUCTVARIANT, UPDATE_PRODUCTVARIANT, DELETE_PRODUCTVARIANT } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   productvariant () {
      return this.$store.state.productvariant.productvariant
    }
  },
  mounted () {
    this.$store.dispatch(GET_PRODUCTVARIANT, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_PRODUCTVARIANT, this.id)
      .then((response) => {
        this.$router.push({ path: '/productvariant' })
      }).catch((err) => {
        this.alert = {
          status: true,
          type: 'error',
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_PRODUCTVARIANT, this.productvariant)
      .then((response) => {
