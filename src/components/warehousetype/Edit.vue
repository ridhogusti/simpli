    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormWarehouseType :WarehouseType="warehousetype"></FormWarehouseType>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/warehousetype'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormWarehouseType from '@/components/warehousetype/Form'
import { GET_WAREHOUSETYPE, UPDATE_WAREHOUSETYPE, DELETE_WAREHOUSETYPE } from '@/store/actions.type'
...
export default {
  ...
  computed: {
   warehousetype () {
      return this.$store.state.warehousetype.warehousetype
    }
  },
  mounted () {
    this.$store.dispatch(GET_WAREHOUSETYPE, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_WAREHOUSETYPE, this.id)
      .then((response) => {
        this.$router.push({ path: '/warehousetype' })
      }).catch((err) => {
        this.alert = {
          status: true,
          type: 'error',
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_WAREHOUSETYPE, this.warehousetype)
      .then((response) => {
