<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/warehousetype/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/warehousetype/create'"></button-create>
...
import { GET_STATE_WAREHOUSETYPES } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.warehousetype.warehousetypes
    },
    pages () {
      return this.$store.state.warehousetype.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.warehousetype.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_WAREHOUSETYPE', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_WAREHOUSETYPES', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_WAREHOUSETYPES', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_WAREHOUSETYPES)
      .then((result) => {
