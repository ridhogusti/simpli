<template>
  <div>
                ...
                <template slot="items" slot-scope="props">
                  <td v-for = "(item,index) in headers" 
                    ...
                    <template v-if="index==0">
                       <router-link :to="'/shift/edit/' + props.item[headers[index].text]">{{ props.item[headers[index].text] }}</router-link>
                    </template>
    ...
    <!-- button create -->
    <button-create :to="'/shift/create'"></button-create>
...
import { GET_STATE_SHIFTS } from '@/store/actions.type'
...
export default {
  ...
  computed: {
    ...
    pages () {
      return this.$store.state.shift.paginations.pageLength
    },
    currentPages () {
      return this.$store.state.shift.paginations.currentPage
    }
  },
  methods: {
    SetAlert (type, msg) {
    ...
    InputPage (page) {
      this.$store.dispatch('GET_PAGINATION_SHIFT', page)
    },
    searchList :_.debounce(function (keyword) {
      this.$store.dispatch('GET_SEARCH_SHIFTS', keyword)
    },500),
    changeSort (column) {
    ...
    GetTableState (sort) {
      this.$store.dispatch('GET_SORTING_SHIFTS', sort)
    }
  },
  created () {
    this.$store.dispatch(GET_STATE_SHIFTS)
      .then((result) => {
