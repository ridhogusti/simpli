    ...
    <v-layout row>
      <v-flex xs12 sm12>
        <v-card>
          ...
          <v-card-text>
            <v-form ref="form">
              <FormShift :Shift="shift"></FormShift>
          ...
          <v-card-text class="text-xs-center pa-0 grey lighten-1">
            <v-btn :to="'/shift'">Back to list</v-btn>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
...
import FormShift from '@/components/shift/Form'
import { GET_SHIFT, UPDATE_SHIFT, DELETE_SHIFT } from '@/store/actions.type'
...
export default {
  ...
  mounted () {
    this.$store.dispatch(GET_SHIFT, this.id)
  },
  methods: {
    deleted () {
      this.$store.dispatch(DELETE_SHIFT, this.id)
      .then((response) => {
    ...
    save () {
      ...
      this.$store.dispatch(UPDATE_SHIFT, this.shift)
      .then((response) => {
